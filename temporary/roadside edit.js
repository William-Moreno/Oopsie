...
setInterval(() => {
  ...
  sqs.receiveMessage(receiveParams, function (err, data) {
    if (err) {
      ...
      // comment out 90-98 to deactivate emails

      // respond to user with console log
      console.log(`${incidentInfo.name}, a roadside assistance vehicle has been dispatched to your location.`);

      // respond to user with SMS

      var smsParams = {
        Message: `${incidentInfo.name}, ${faker.name.findName()} with ZZZ Roadside Assistance has been dispatched to your location.`,
